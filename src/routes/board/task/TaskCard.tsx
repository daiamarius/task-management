    ...
    return (
        <Card className={'flex flex-col justify-between relative'}>
            <CardHeader>
                <CardTitle className={'text-sm flex flex-col gap-1'}>
                    <NavLink
                        to={`/task/${task.id}`}
                        className={
                            'text-primary hover:underline transition line-clamp-2 h-[42px]'
                        }
                    >
                    ...
                    <div className={'inline-flex justify-between items-center'}>
                        ...
                        <div className={'flex flex-col'}>
                            {task.createdAt !== task.updatedAt && (
                                <div
                                    className={
                                        'text-xs font-light text-muted-foreground shrink-0 flex gap-1 items-center'
                                    }
                                >
                                    Updated:
                                    <LuCalendar />
            ...
            {isFetching && (
                <Progress
                    indeterminate={true}
                    className={'h-1 absolute rounded-t-none bottom-0'}
                />
            )}
        </Card>
    )
